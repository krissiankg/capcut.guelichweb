...
import { isEmailConfigured } from "./email/resend.js";
...
app.get("/verification-en-attente", (_req, res) => {

  res.sendFile(path.join(publicDir, "verification-pending.html"));
...
app.get("/api/health", async (_req, res) => {
  ...
  try {
    const { error } = await supabase.from("capcut_plans").select("id").limit(1);
    if (error) {
  ...
  const statusCode = payload.db === "ok" ? 200 : 503;
